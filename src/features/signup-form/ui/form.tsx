      ...
      <Fieldset>
        ...
        <form.Field name="email" validators={{ onChange: z.email( 'Invalid email address' ) }} children={field =>
          <Field className="block mt-8 relative">
            <Input id={field.name} name={field.name} value={field.state.value} onChange={e => field.handleChange( e.target.value )} data-error={field.state.meta.isValid ? false : true} type="email" placeholder="name@company.com" className="peer w-full p-3.5 pl-11 rounded-xl bg-zinc-100 placeholder:text-zinc-400/50 data-[error=true]:bg-rose-50" />
            <EmailIcon className="w-4 h-4 fill-zinc-400/50 peer-focus:fill-black absolute top-1/2 left-4 -translate-y-1/2" aria-hidden="true" />
            {!field.state.meta.isValid && <Description className="px-4 text-xs text-rose-500 absolute -bottom-5">{field.state.meta.errors.map( error => error?.message ).join( ', ' )}</Description>}
          </Field> }
        />
        <form.Field name="password" validators={{ onChange: z.string().min( 8, 'Must be at least 8 characters long' ) }} children={field =>
          <Field className="block mt-8 relative">
            <Input id={field.name} name={field.name} value={field.state.value} onChange={e => field.handleChange( e.target.value )} data-error={field.state.meta.isValid ? false : true} type={isPasswordVisible ? 'text' : 'password'} placeholder="•••••••••" className="peer w-full p-3.5 pl-11 rounded-xl bg-zinc-100 placeholder:text-zinc-400/50 data-[error=true]:bg-rose-50" />
            <LockIcon className="w-4 h-4 fill-zinc-400/50 peer-focus:fill-black absolute top-1/2 left-4 -translate-y-1/2" aria-hidden="true" />
            <Button onClick={handlePasswordClick} type="button" className="w-5 h-5 absolute top-1/2 right-4 -translate-y-1/2 cursor-pointer">
              {isPasswordVisible ? <OpenEyeIcon className="w-5 h-5 fill-zinc-400/50" /> : <ClosedEyeIcon className="w-5 h-5 fill-zinc-400/50" />}
            </Button>
            {!field.state.meta.isValid && <Description className="px-4 text-xs text-rose-500 absolute -bottom-5">{field.state.meta.errors.map( error => error?.message ).join( ', ' )}</Description>}
          </Field> }
        />
