...
const SignInForm: FC = () => {
  ...
  return (
    <form onSubmit={handleSubmitForm} className="min-w-xs">
      <Fieldset>
        ...
        <form.Field name="password" children={field =>
          <Field className="block mt-8 relative">
            <Input id={field.name} name={field.name} value={field.state.value} onChange={e => field.handleChange( e.target.value )} type="password" placeholder="•••••••••" className="peer w-full p-3.5 pl-11 rounded-xl bg-[#f5f5f5] placeholder:text-[#C2C3CB] data-[error=true]:outline-2" />
            <svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" className="w-4 h-4 fill-[#C2C3CB] peer-focus:fill-black absolute top-1/2 left-4 -translate-y-1/2" aria-hidden="true">
        ...
        />
        <Button type="submit" className="w-full mt-8 p-3.5 rounded-xl bg-black text-white cursor-pointer outline-offset-3">Sign in</Button>
      </Fieldset>
    </form>
  );
};
